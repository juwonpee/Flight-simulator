...
namespace MapMagic.Previews
{
	public class ObjectsPreview : IPreview
	{
		...
		#region Generate
			...
			{ 
				if (data != null)
					SetObject((TransitionsList)data.products[outlet], data.heights, data.area); 
			}
			...
			public void ExecuteInThread ()
			{
				...
				for (int t=0; t<trns.count; t++)
				{
					...
					if (heights != null) pos.y = pos.y/heights.worldSize.y; //heights is null on first generate
