...
namespace MapMagic.Nodes.GUI
{
	...
	{	
		...
		public static void DrawCurve (Curve curve, float[] histogram)
		{
			...
			if (UI.current.scrollZoom.zoom > 0.75f) CurveDraw.DragCurve(curve);
		...
		/// Just draws curve without dragging nodes
		{
			...
			//drawing curve nodes
			if (UI.current.scrollZoom.zoom > nodesDraggedZoom)
				for (int i=0; i<curve.points.Length; i++)
			{
			...
			curveMat.SetVector("_Rect", Cell.current.GetRect(UI.current.scrollZoom).ToV4());
		...
		/// Older way to display a curve without shader
		{
			...
			int numSubSegments = (int)(maxSubSegments*UI.current.scrollZoom.zoom);
		...
		private static void DrawPoint (Curve.Node node)
		{
			...
			Vector2 dispPos = UI.current.scrollZoom.ToScreen(cellPos);
		...
		private static void DrawSegment (Curve.Node prevNode, Curve.Node nextNode, Vector3[] posArray, Cell cell)
		{
			...
			for (int i=0; i<pointsNum; i++)
				posArray[i] = UI.current.scrollZoom.ToScreen( ToCell(posArray[i]) )  +  new Vector2(0.5f, 0.5f); //adding 0.5 to make the line center-sized at pixel
		...
		{
			posArray[0] = UI.current.scrollZoom.ToScreen( ToCell(start) );
			posArray[1] = UI.current.scrollZoom.ToScreen( ToCell(end) );
