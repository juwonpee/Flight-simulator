...
{
    public float engine_coeffecient_min_speed;
    public float engine_coeffecient_max_speed;
    public float engine_coeffecient_min;
    public float engine_coeffecient_max;
    ...
    private void FixedUpdate()
    {
        ...
        // Calculate thrust coeffecient
        float thrustCoeffecient = 0;
        Vector3 airspeed = (float)1.94384 * transform.GetComponent<FlightStats>().airspeed;
        if (airspeed.z < engine_coeffecient_min_speed) thrustCoeffecient = engine_coeffecient_min;
        else if (airspeed.z >= engine_coeffecient_min_speed && airspeed.z < engine_coeffecient_max_speed) {
            // Find proportion
            // TODO: Proportion calculation wrong here
            thrustCoeffecient = engine_coeffecient_min + (airspeed.z - engine_coeffecient_min_speed) / (engine_coeffecient_max_speed - engine_coeffecient_min_speed);
        }
        else thrustCoeffecient = engine_coeffecient_max;


        Vector3 velocityPrediction = PredictVelocity(forceAndTorqueThisFrame.p + transform.forward * thrust * thrustCoeffecient* thrustPercent + Physics.gravity * rb.mass);
        ...
        rb.AddForce(transform.forward * thrust * thrustCoeffecient * thrustPercent);
    }
