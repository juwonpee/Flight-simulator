...
namespace MapMagic.Core 
{
	...
	{
		public static readonly SemVer version = new SemVer(2,0,5); 
		...
		public int draftSwitchRange = 1; //actually Main range
		...
		public void Update () 
		{ 
			tiles.Update((Vector3)tileSize, this, distsOnly: !isPlaying); //distsOnly: only updating distance priority in editor
		...
		#region IMapMagic
			...
			public void Clear (Generator gen)
			{
				...
				foreach (TerrainTile tile in tiles.All())
				{
					if (tile.main?.data!=null) //graph.Clear(gen, tile.main.data);
					{
						tile.main.data.ready.Remove(gen, tile.main.data.subDatas);
						graph.ClearChanged(tile.main.data);
					}

					if (tile.draft?.data!=null) //graph.Clear(gen, tile.draft.data);
					{
						tile.draft.data.ready.Remove(gen, tile.draft.data.subDatas);
			...
			public void ClearAll ()
			{
				foreach (TerrainTile tile in tiles.All())
				{
					...
					tile.main?.data?.Clear();
					tile.draft?.data?.Clear(); 
				}
			}
