...
namespace Den.Tools.Matrices {
	...
	/// The ones that require Stripes
	{
		...
		#region Stripe Readings/Writings
			...
			public static void ReadDiagonal (Stripe stripe, Matrix matrix, int x, int z, int stepX = 1, int stepZ = 1) 
			{
				int start = (z-matrix.rect.offset.z)*matrix.rect.size.x - matrix.rect.offset.x  +  x;

				int xLength = stepX>0  ?  matrix.rect.offset.x+matrix.rect.size.x - x  :  x - matrix.rect.offset.x;
				int zLength = stepZ>0  ?  matrix.rect.offset.z+matrix.rect.size.z - z  :  z - matrix.rect.offset.z;
				stripe.length = stripe.length<xLength ? stripe.length : xLength;
				stripe.length = stripe.length<zLength ? stripe.length : zLength;

				for (int i=0; i<stripe.length; i++)
					stripe.arr[i] = matrix.arr[start + i*stepX + i*matrix.rect.size.x*stepZ];
			}

			public static void WriteDiagonal (Stripe stripe, Matrix matrix, int x, int z, int stepX = 1, int stepZ = 1) 
			{
				int start = (z-matrix.rect.offset.z)*matrix.rect.size.x - matrix.rect.offset.x  +  x;

				for (int i=0; i<stripe.length; i++)
					matrix.arr[start + i*stepX + i*matrix.rect.size.x*stepZ] = stripe.arr[i];
			}

			public static void MaxDiagonal (Stripe stripe, Matrix matrix, int x, int z, int stepX = 1, int stepZ = 1) 
			{
				int start = (z-matrix.rect.offset.z)*matrix.rect.size.x - matrix.rect.offset.x  +  x;

				for (int i=0; i<stripe.length; i++)
				{
					float matrixVal = matrix.arr[start + i*stepX + i*matrix.rect.size.x*stepZ];
					float lineVal = stripe.arr[i];

					if (lineVal>matrixVal) 
						matrix.arr[start + i*stepX + i*matrix.rect.size.x*stepZ] = lineVal;
				}
			}
		...
		#region Free Resize
			...
			// Shrinking both dimensions (using temporary array)
			{
				...
				private static extern void UpsizeHorizontally (Matrix src, Matrix dst, Stripe srcStripe, Stripe dstStripe);

				[DllImport ("NativePlugins", CallingConvention = CallingConvention.Cdecl, EntryPoint = "MatrixOps_UpsizeVertically")]
				private static extern void UpsizeVertically (Matrix src, Matrix dst, Stripe srcStripe, Stripe dstStripe);

				[DllImport ("NativePlugins", CallingConvention = CallingConvention.Cdecl, EntryPoint = "MatrixOps_ResampleStripeCubic")]
				private static extern void ResampleStripeCubic (Stripe src, Stripe dst);
				...
				private static void UpsizeHorizontally (Matrix src, Matrix dst, Stripe srcStripe, Stripe dstStripe)
				{
					srcStripe.length = src.rect.size.x;
					dstStripe.length = dst.rect.size.x;

					int min = src.rect.offset.z;
					int max = src.rect.offset.z + src.rect.size.z;
				...
				private static void UpsizeVertically (Matrix src, Matrix dst, Stripe srcStripe, Stripe dstStripe)
				{
					srcStripe.length = src.rect.size.z;
					dstStripe.length = dst.rect.size.z;

					int min = src.rect.offset.x;
					int max = src.rect.offset.x + src.rect.size.x;
				...
				private static void ResampleStripeCubic (Stripe src, Stripe dst)
				/// Scales the stripe filling dst with interpolated values. Cubic for upscale
				{
					for (int dstX=0; dstX<dst.length; dstX++)
					{
						float srcX = ((float)dstX / dst.length) * src.length;
		...
		#region ResizeFast
			...
			/// Blends all mipmaps in one matrix
			{
				...
				public static extern void DownscaleFast (Matrix src, Matrix dst);

				[DllImport ("NativePlugins", CallingConvention = CallingConvention.Cdecl, EntryPoint = "MatrixOps_DownscaleFastTmp")]
				public static extern void DownscaleFast (Matrix src, Matrix dst, Matrix tmp, Stripe srcStripe, Stripe dstStripe);
				...
				/// Does not require temp matrix
				{
					...
					srcStripe.length = src.rect.size.z;
					dstStripe.length = dst.rect.size.z;
					for (int x=dst.rect.offset.x+dst.rect.size.x-1; x>=dst.rect.offset.x; x--) //inverse order to re-use the same array
					{
				...
				/// Only for cases when dst rect size is exactly twice smaller than src
				{
					...
					for (int z=src.rect.offset.z; z<src.rect.offset.z+src.rect.size.z; z++)
					{
						...
						WriteLine(dstStripe, tmp, tmp.rect.offset.x, z);
					}
					...
					{
						ReadRow(srcStripe, tmp, x, dst.rect.offset.z);
						ResampleStripeDownFast(srcStripe, dstStripe);
						WriteRow(dstStripe, dst, x, src.rect.offset.z);
					}
				}
				...
				/// Like cubic, works faster, but requires dst.size = src.size*2
				{
					for (int srcX=0; srcX<src.length-1; srcX++)
					{
						...
						dst.arr[srcX*2+1] = src.arr[srcX]*0.5f + src.arr[srcX+1]*0.5f;
					}
		...
		#region Normals/Delta
				...
				[DllImport ("NativePlugins", CallingConvention = CallingConvention.Cdecl, EntryPoint = "MatrixOps_NormalRelief")]
				public static extern void NormalRelief(Matrix src, Matrix dst, float horizontalHeight = 1, float verticalHeight = 1);
			...
			public static void NormalRelief (Matrix src, Matrix dst, float horizontalHeight=1, float verticalHeight=1)
			/// To generate the normal map of a heightmap
			{
				...
				{
					ReadLine(stripe, src, rect.offset.x, z);
					NormalStripe(stripe, horizontalHeight);
					WriteLine(stripe, dst, rect.offset.x, z);
				}
				...
				for (int x=min.x; x<max.x; x++)
				{
					...
					NormalStripe(stripe, verticalHeight);
					AddRow(stripe, dst, x, rect.offset.z);
				}
			}
			...
			private static void NormalStripe (Stripe stripe, float height)
		...
		#region Spread
				...
				public static extern void Spread(Matrix src, Matrix dst, float subtract = 0.01f, float multiply = 1.0f);

				[DllImport ("NativePlugins", CallingConvention = CallingConvention.Cdecl, EntryPoint = "MatrixOps_SpreadLinear")]
				public static extern void SpreadLinear(Stripe stripe, float subtract = 0.01f, float regardSmallerValues = 1);
			...
			public static void Spread (Matrix src, Matrix dst, float subtract=0.01f, float multiply=1f)
			...
			/// Will overwrite the gray values with the bigger spread values, so couldn't be used as padding
			{
				...
				{
					ReadLine(stripe, src, src.rect.offset.x, z);
					SpreadLinear(stripe, subtract, regardSmallerValues:1);
				...
				for (int z=min.z; z<max.z; z++)
				{
					...
					SpreadLinear(stripe, subtract, regardSmallerValues:0);
					MaxLine(stripe, dst, src.rect.offset.x, z);
				}
			...
			private static void SpreadLinear (Stripe stripe, float subtract=0.01f, float regardSmallerValues=1)
			/// Spreads higher values (whites) over the lower (darker) ones
			/// regardSmallerValues is a factor forcing smaller values (like antialising) be considered. 0 will generate the same result each iteration.
			{
				SpreadLinearRight(stripe, subtract, regardSmallerValues);
				SpreadLinearLeft(stripe, subtract, regardSmallerValues);
			}
			...
			private static void SpreadLinearRight (Stripe stripe, float subtract=0.01f, float regardSmallerValues=1)
			{
				...
				for (int x=1; x<stripe.length; x++)
				{
					...
					if (prevVal > currVal)
					{
						currVal = prevVal - subtract + (currVal/prevVal)*subtract*regardSmallerValues;
...
//					leftStripe.arr[x] = leftMask.arr[x];
		...
		#region Lock Padding
			...
			public static void ExtendCircular (this Matrix matrix, Coord center, int radius, int extendRange, int expandPixels=0)
			{
				...
				Stripe leftStripe = new Stripe((radius+extendRange+1)*2*4 + 1);
				Stripe rightStripe = new Stripe(leftStripe.length);
				Stripe leftMask = new Stripe(leftStripe.length);
				Stripe rightMask = new Stripe(leftStripe.length);
				...
				//[DllImport ("NativePlugins", CallingConvention = CallingConvention.Cdecl, EntryPoint = "MatrixOps_ExtendCircular")]
				//public static extern void ExtendCircular(this Matrix matrix, Coord center, int radius, int extendRange, int expandPixels=0);
