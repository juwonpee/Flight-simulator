...
namespace MapMagic.Previews
{
	public class SplinePreview : IPreview
	{
		...
		#region Terrain
			...
			public void ToTerrain (Terrain terrain)
			{
		...
		#region Window
			...
			public class SplinePreviewWindow : BaseMatrixWindow, IPreviewWindow
			{
				...
				public Guid SerializedGenGuid 
				{ 
					get{ return new Guid(serializedGenGuidArr); }
					set{ serializedGenGuidArr = value.ToByteArray(); }
				}
				public byte[] serializedGenGuidArr; 
		...
		#region GUI
			...
			[Obsolete] public void DrawBeizer ()
			{
				if (Event.current.type != EventType.Repaint) return;

				Rect cellRect = Cell.current.GetRect(UI.current.scrollZoom);
				Vector2 cellSize = cellRect.size;
				Vector2 cellOffset = cellRect.position;

				Vector3 tileSize = (Vector3)worldSize;

				Material mat = UI.current.textures.GetMaterial("MapMagic/ObjectGLPreview"); 
				mat.SetVector("_ClipRect", new Vector4(cellOffset.x, cellOffset.y, cellOffset.x+cellSize.x, cellOffset.y+cellSize.y));

				foreach (Line line in splineSys.lines)
					for (int s=0; s<line.segments.Length; s++)
					{
						Vector3 pos1 = line.segments[s].start.pos;
						pos1 = new Vector3 (
							1f*pos1.x/tileSize.x * cellSize.x + cellOffset.x, 
							cellSize.y - 1f*pos1.z/tileSize.z * cellSize.y + cellOffset.y,
							0);

						Vector3 pos2 = line.segments[s].end.pos;
						pos2 = new Vector3 (
							1f*pos2.x/tileSize.x * cellSize.x + cellOffset.x, 
							cellSize.y - 1f*pos2.z/tileSize.z * cellSize.y + cellOffset.y,
							0);

						Vector3 tan1 = line.segments[s].start.dir;
						tan1 = new Vector3 (
							1f*tan1.x/tileSize.x * cellSize.x, 
							1f*tan1.z/tileSize.z * cellSize.y,
							0);

						Vector3 tan2 = line.segments[s].end.dir;
						tan2 = new Vector3 (
							1f*tan2.x/tileSize.x * cellSize.x , 
							1f*tan2.z/tileSize.z * cellSize.y,
							0);

						Handles.DrawBezier(pos1, pos2, pos1+tan1, pos2+tan2, 
							new Color(1, 0.5f, 0, 1), null, 2);
