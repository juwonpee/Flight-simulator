...
namespace MapMagic.Locks
{
	public static class LockDraw
	{
		...
		public static void DrawSceneGUI (MapMagicObject mapMagic)
		{
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				for (int l=0; l<mapMagic.locks.Length; l++)
				{
					...
					lockArea.guiHeight = DrawCircle(polyLine, lockArea.worldPos, lockArea.worldRadius, color, corners, terrains, terrainRects);
					DrawCircle(polyLine, lockArea.worldPos, lockArea.worldRadius+lockArea.worldTransition, tcolor, corners, terrains, terrainRects);
				}
		...
		public static float DrawCircle (PolyLine line, Vector3 center, float radius, Color color, Vector3[] corners, List<Terrain> terrains, List<Rect> terrainRects)
		/// It will return an average height BTW almost for free :)
		{
			...
			line.DrawLine(corners, color, lineThickness, zMode:PolyLine.ZMode.Overlay, offset:0.1f);
