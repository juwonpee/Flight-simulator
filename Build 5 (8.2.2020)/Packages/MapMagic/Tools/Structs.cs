...
namespace Den.Tools 
{
	...
	public struct CoordRect
	{
		...
		public static CoordRect Intersected (CoordRect c1, CoordRect c2) { c1.Clamp(c2.Min, c2.Max); return c1; }
		...
		public static CoordRect WorldToGridRect (Vector3 worldPos, Vector3 worldSize, float cellSize)
		/// Converts world rect to 0-aligned grid rect
		/// Note that worldPos and worldSize are re-calculated to align to grid
		{
			if (cellSize == 0) 
				throw new Exception("Cell size is zero");

			Coord rectMin = new Coord( Mathf.FloorToInt(worldPos.x/cellSize), Mathf.FloorToInt(worldPos.z/cellSize) );
			Coord rectMax = new Coord( Mathf.CeilToInt((worldPos.x+worldSize.x)/cellSize), Mathf.CeilToInt((worldPos.z+worldSize.z)/cellSize) );

			return new CoordRect(rectMin, rectMax-rectMin);
		}

		public static CoordRect WorldToGridRect (ref Vector3 worldPos, ref Vector3 worldSize, float cellSize)
		/// Converts world rect to 0-aligned grid rect
		/// Note that worldPos and worldSize are re-calculated to align to grid
		{
			CoordRect rect = WorldToGridRect(worldPos, worldSize, cellSize);
	...
	public struct Vector2D : IEquatable<Vector2D> 
	{
		...
		public static float SqrMagnitude (Vector3 v) { return v.x*v.x + v.z*v.z; }
