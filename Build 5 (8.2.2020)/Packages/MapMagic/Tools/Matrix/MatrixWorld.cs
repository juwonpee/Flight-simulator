...
namespace Den.Tools.Matrices
{
	...
	public class MatrixWorld : Matrix, ICloneable
	{
		...
		public Vector2D PixelSize {get{ return new Vector2D(worldSize.x/rect.size.x, worldSize.z/rect.size.z); }}
		...
		#region World to Pixel / Pixel to World
			...
			public Vector3 PixelToWorld (int x, int z, bool center=false)
			/// Finding pixel center if Center enabled
			{
				//finding relative percent
				float percentX = 1f * (x + (center ? 0.5f : 0) - rect.offset.x) / rect.size.x;
				float percentZ = 1f * (z + (center ? 0.5f : 0) - rect.offset.z) / rect.size.z;
			...
			/// Creates pixel CoordRect that includes world rect (size is always ceiled)
			{
				...
				return new CoordRect(pOffset, pMax-pOffset+1);
			}
