...
namespace MapMagic.Terrains
{
	...
	public static class Weld
	{
		...
		#region Weld in Thread
			...
			{
				HeightOutput200.ApplySetData heightDataFull = thisData.apply.Get<HeightOutput200.ApplySetData>();
				if (heightDataFull != null)
					thisEdges.heightEdges.ReadFloats2D(heightDataFull.heights2D);

				HeightOutput200.ApplySplitData heightSplitData = thisData.apply.Get<HeightOutput200.ApplySplitData>();
				if (heightSplitData != null)
					thisEdges.heightEdges.ReadSplitFloats2D(heightSplitData.heights2DSplits);

				#if UNITY_2019_1_OR_NEWER
				HeightOutput200.ApplyTexData heightTexData = thisData.apply.Get<HeightOutput200.ApplyTexData>();
				if (heightTexData != null)
					thisEdges.heightEdges.ReadRawFloat(heightTexData.texBytes);
				#endif


				TexturesOutput200.ApplyData texturesData = thisData.apply.Get<TexturesOutput200.ApplyData>();
		...
		#region Weld on LOD switch
			...
			/// On drafts only because of the performance reasons
			{
				...
				for (int d=0; d<weldDirections.Length; d++)
				{
					...
					if (!neigTile.draft.applyReady || !neigTile.draft.generateReady) continue; //TODO: test this
