	...
	public class MapMagicInspector : Editor
	{
		...
		{	
			...
			if (mapMagic.guiTiles)
				PinDraw.DrawSceneGUI(mapMagic, selectionMode);
			...
			using (Cell.LineStd)
				using (new Draw.FoldoutGroup(ref mapMagic.guiInfiniteTerrains, "Infinite Terrain (Playmode)", isLeft:true))
					if (mapMagic.guiInfiniteTerrains)
					{
						...
						{
							using (Cell.LineStd) Draw.Field(ref mapMagic.draftSwitchRange, "Main Range");
							using (Cell.LineStd) Draw.Field(ref mapMagic.tiles.generateRange, "Drafts Range");
						}
						else
							using (Cell.LineStd) Draw.Field(ref mapMagic.tiles.generateRange, "Generate Range");
			...
			using (Cell.LineStd)
				using (new Draw.FoldoutGroup(ref mapMagic.guiTileSettings, "Tile Settings", isLeft:true))
					if (mapMagic.guiTileSettings)
					{
						...
						using (Cell.LineStd) Draw.ToggleLeft(ref mapMagic.draftsInPlaymode, "Playmode");
