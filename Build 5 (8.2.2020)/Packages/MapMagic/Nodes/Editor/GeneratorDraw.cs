...
//			using (Timer.Start("Field"))
			...
			//layers
			if (gen is ILayered<object> layGen)
//				using (Timer.Start("Layers"))
			{
				using (Cell.LinePx(0))
					DrawLayers(layGen);
			}
...
//				using (Timer.Start("Footer"))
		...
		/// Internal move function for DragGenerator
		{
			...
			moveTo.x = (int)(float)(moveTo.x*dpiFactor + 0.50002f) / dpiFactor;
			moveTo.y = (int)(float)(moveTo.y*dpiFactor - 0.50002f) / dpiFactor;
		...
		public static void DrawLayers (ILayered<object> layGen)
		{
			object[] layers = layGen.Layers;
			bool inversed = layGen is IInvLayered<object>;
			...
			void Add (int num)
			{
				...
				//finding layer type
				Type layerType = GetLayersType(layGen);
				if (layerType == null)
					throw new Exception($"Could not create layer type for {layGen.GetType()}");

				//creating layer
				object layer = Activator.CreateInstance(layerType);
				//object layer = new T(); // layGen.CreateLayer();

				if (layer is IInlet<object> inletLayer) inletLayer.SetGen((Generator)layGen);
				if (layer is IOutlet<object> outletLayer) outletLayer.SetGen((Generator)layGen);

				ArrayTools.Insert(ref layers, num, layer);

				layGen.SetLayers(layers);
			}
			...
			void DrawLayer (int num)
			{
				...
				Draw.Editor(layers[num], new object[] {num, layGen} );
			}
			...
			using (Cell.LinePx(20)) 
				LayersEditor.DrawAddRemove(layersCell, "Layers", Add, Remove, Move);

			using (Cell.LinePx(0)) 
				using (Cell.Padded(-1,-1,0,0))
					LayersEditor.DrawLayersThemselves(layersCell, layers.Length, DrawLayer, roundBottom:false);

			if (Cell.current.valChanged  &&  GraphWindow.current.mapMagic!=null)
				GraphWindow.RefreshMapMagic((Generator)layGen);
		}

		private static Type GetLayersType (ILayered<object> layGen)
		{
			Type layerType = null;
			Type genType = layGen.GetType();
			Type[] interfaces = genType.GetInterfaces();
			foreach (Type itype in interfaces)
			{
				if (!itype.IsGenericType) continue;
				if (!typeof(ILayered<object>).IsAssignableFrom(itype)) continue;
				layerType = itype.GenericTypeArguments[0];
			}
			return layerType;
		}
