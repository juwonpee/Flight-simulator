			...
			float[] histogram = null)
		{
			...
			using (Cell.Line) 
			{
				using (Cell.RowPx(diamondHeight))
				{
					...
					outMin = VerDiamond(outMin, minTex, clickStart:0, clickEnd:(outMin+outMax)/2, min:0, max:outMax);
				}
				...
				using (Cell.RowPx(diamondHeight)) 
				{
					...
					outMax = VerDiamond(outMax, maxTex, clickStart:(outMin+outMax)/2, clickEnd:1, min:outMin, max:1);
				}
			}
