...
namespace MapMagic.Nodes.MatrixGenerators
{
	...
	public class  HeightOutput200 : Generator, IInlet<MatrixWorld>, IOutputGenerator, IOutput
	{
		//[Val("Inlet", "Inlet")] public readonly Inlet<MatrixWorld> srcIn = new Inlet<MatrixWorld>();

		//public override IEnumerable<IInlet<object>> Inlets() { yield return srcIn; }
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			//adding to finalize
			data.finalize.Add(finalizeAction, this, src, data.currentBiomeMask); //adding src since it's not changing

			#if MM_DEBUG
			Debug.Log("HeightOut Generated");
			#endif
		}


		public static Matrix GetInterpolated (Matrix srcHeights, Interpolation interpolation)
		///Resizes/Blurs heightmap according to interpolation settings (returns dst if no resize/blur)
		{
			int upscale;
			switch (interpolation)
			{
				case Interpolation.Scale2X: upscale=2; break;
				case Interpolation.Scale4X: upscale=4; break;
				default: upscale=1; break;
			}

			Matrix dstHeights;
			if (upscale==1) dstHeights = srcHeights;
			else 
			{
				dstHeights = new Matrix( new CoordRect(srcHeights.rect.offset, srcHeights.rect.size*upscale) ); 
				MatrixOps.Resize(srcHeights, dstHeights);
				MatrixOps.GaussianBlur(dstHeights, 1); //yes, blurring data.heights
			}
		...
		public static void Finalize (TileData data, StopToken stop)
		{
			...
			else //if TextureToHeightmap
			{
				...
				applyData = new ApplyTexData() { res=res, margins=margins, splitSize=splitSize, height=data.globals.height, texBytes=bytes }; 
			}
			#else
			else
			...
			//pushing to apply
			if (stop!=null && stop.stop) return;
			Graph.OnBeforeOutputFinalize?.Invoke(typeof(HeightOutput200), data, applyData, stop);
			data.apply.Add(applyData);

			#if MM_DEBUG
			Debug.Log("HeightOut Finalized");
			#endif
		}


		void IOutputGenerator.Purge (TileData data, Terrain terrain) { Purge(data,terrain); }
		public static void Purge (TileData data, Terrain terrain)
		{
		...
		public class ApplySetData : IApplyData, IApplyHeightData
		{
			...
			public void Apply (Terrain terrain)
			{
				...
				#if MM_DEBUG
				Debug.Log("HeightOut Applied Set");
				#endif
			}
		...
		public class ApplySplitData : IApplyDataRoutine, IApplyHeightData
		{
			...
			public IEnumerator ApplyRoutine (Terrain terrain)
			{
				...
				#if MM_DEBUG
				Debug.Log("HeightOut Applied Split");
				#endif
			}
		...
		public class ApplyTexData : IApplyDataRoutine, IApplyHeightData
		{
			...
			public IEnumerator ApplyRoutine (Terrain terrain)
			{
				...
				#if MM_DEBUG
				Debug.Log("HeightOut Applied Texture");
				#endif
			}
