...
namespace Den.Tools.GUI
{
	public static class Draw
	{
		...
		const int vectorXYWidth = 15; //width of 'width' and 'height' elements in vector field
		...
		#region Fast Fields
			...
			public static float Field (float val) 
			{
				if (UI.current.layout) return val;
				if (UI.current.optimizeElements && !UI.current.IsInWindow()) return val;

				//Cell.current.name = "FloatField";

				if (Cell.current.disabled) UnityEditor.EditorGUI.BeginDisabledGroup(true);

				Rect rect = Cell.current.GetRect(UI.current.scrollZoom, padding:fieldPadding);
				float newVal = EditorGUI.DelayedFloatField(rect, val, style:UI.current.styles.field);  
			...
			public static float Field (float val, GUIStyle style) 
			{
				if (UI.current.layout) return val;
				if (UI.current.optimizeElements && !UI.current.IsInWindow()) return val;
				...
				Rect rect = Cell.current.GetRect(UI.current.scrollZoom, padding:fieldPadding);
				float newVal = EditorGUI.DelayedFloatField(rect, val, style:style);
			...
			public static float Field (float val, string label) 
			{
			...
			public static int Field (int val) 
			{
				if (UI.current.layout) return val;
				if (UI.current.optimizeElements && !UI.current.IsInWindow()) return val;
				...
				Rect rect = Cell.current.GetRect(UI.current.scrollZoom, padding:fieldPadding);
				//int newVal = EditorGUI.IntField(rect, val, style:UI.current.styles.field); //IntField generates 0.015Kb garbage each
				int newVal = (int)EditorGUI.DelayedFloatField(rect, val, style:UI.current.styles.field);
			...
			public static int Field (int val, string label) 
			{
				if (UI.current.optimizeCells && !UI.current.IsInWindow())
					{ Cell.current.Skip(); return val; }
		...
		#region Other Fields
			...
			private static double DoubleFieldFn (Rect rect, double val) { return EditorGUI.DelayedDoubleField(rect, val, style:UI.current.styles.field); }
			private static readonly Func<Rect,double,double> doubleFieldFn = (rect,val) => EditorGUI.DelayedDoubleField(rect, val, style:UI.current.styles.field);
			...
			public static object UniversalField (object val, Type type, string label)
			{
				...
				{
					Label(label);
					if (type == typeof(int)) val = DragValue((int)val);
					if (type == typeof(float)) val = DragValue((float)val);
				}
				
				using (Cell.RowRel(Cell.current.fieldWidth)) 
					val = UniversalField(val, type);
		...
		#region Generic Object Fields
			...
			public static UnityEngine.Object ObjectField<T> (T val, bool allowSceneObject=false) where T: UnityEngine.Object 
		...
		#region Vectors
			...
			public static Vector2 Field (Vector2 val)
			{
				...
				using (Cell.LineStd) val.y = Field(val.y, "Y");
			...
			public static Vector2 Field (Vector2 val, string label)
			{
				...
				using (Cell.RowRel(1-Cell.current.fieldWidth))
				{
					...
					{
						using (Cell.LineStd) { Label("X"); val.x = DragValue(val.x); }
						using (Cell.LineStd) { Label("Y"); val.y = DragValue(val.y); }
					}
				}
				using (Cell.RowRel(Cell.current.fieldWidth))
				{
			...
			public static Vector2 Field (Vector2 val, string label, string xName, string yName, int xyWidth=15)
			{
				...
				using (Cell.RowRel(1-Cell.current.fieldWidth))
				{
					...
					{
						using (Cell.LineStd) { Label(xName); val.x = DragValue(val.x); }
						using (Cell.LineStd) { Label(yName); val.y = DragValue(val.y); }
					}
				}
				using (Cell.RowRel(Cell.current.fieldWidth))
				{
			...
			public static Vector3 Field (Vector3 val, string label)
			{
				...
				using (Cell.RowRel(1-Cell.current.fieldWidth))
				{
					...
					using (Cell.RowPx(vectorXYWidth))
					{
						using (Cell.LineStd) { Label("X"); val.x = DragValue(val.x); }
						using (Cell.LineStd) { Label("Y"); val.y = DragValue(val.y); }
						using (Cell.LineStd) { Label("Z"); val.z = DragValue(val.z); }
					}
				}
				using (Cell.RowRel(Cell.current.fieldWidth))
				{
					using (Cell.LineStd) val.x = Field(val.x);
					using (Cell.LineStd) val.y = Field(val.y);
					using (Cell.LineStd) val.z = Field(val.z);
				}
			...
			public static Vector4 Field (Vector4 val)
			{
				...
				using (Cell.LineStd) val.y = Field(val.y, "Y");
				using (Cell.LineStd) val.z = Field(val.z, "Z");
				using (Cell.LineStd) val.w = Field(val.w, "W");
			...
			public static Vector4 Field (Vector4 val, string label)
			{
				...
				using (Cell.RowRel(1-Cell.current.fieldWidth))
				{
					...
					using (Cell.RowPx(vectorXYWidth))
					{
						...
						using (Cell.LineStd) { Label("Y"); val.y = DragValue(val.y); }
						using (Cell.LineStd) { Label("Z"); val.z = DragValue(val.z); }
						using (Cell.LineStd) { Label("W"); val.w = DragValue(val.w); }
					}
				}
				using (Cell.RowRel(Cell.current.fieldWidth))
				{
					...
					using (Cell.LineStd) val.y = Field(val.y);
					using (Cell.LineStd) val.z = Field(val.z);
					using (Cell.LineStd) val.w = Field(val.w);
				}
		...
		#region Class
			...
			public static void Class (object obj, string category=null, bool addFieldsToCellObjs=false) 
			/// Draws all values of the class marked with Val attribute (and category)
			/// If cellFields defined fills cell to fieldInfo dictionary
			{
				...
				for (int a=0; a<attributes.Length; a++)
				{
					...
					try { ClassField(att.field, att.type, obj, att.name, att.isLeft, att.allowSceneObject); }
					finally { cell.Dispose(); }

					if (addFieldsToCellObjs)
						UI.current.cellObjs.ForceAdd(att.field, cell, "Field");
				}
			}


			public static void ClassField (FieldInfo field, Type type, object obj, string name, bool isLeft=false, bool allowSceneObject=false)
			{
				...
				if (type == typeof(float))
				{
					...
					{
						float val = Getter<float>.GetGetter(field)(obj);
						val = Field(val, name);
						if (Cell.current.valChanged)  field.SetValue(obj, val); 
					}
				}
				...
				else if (type == typeof(int))
				{
					...
					{
						int val = Getter<int>.GetGetter(field)(obj);
						val = Field(val, name);
						if (Cell.current.valChanged)  field.SetValue(obj, val); 
					}
				}
				...
				{
					bool val = Getter<bool>.GetGetter(field)(obj);

					if (!isLeft) val = Toggle(val, name);
					else val = ToggleLeft(val, name);

					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				else if (type == typeof(string))
				{
					...
					{
						string val = Getter<string>.GetGetter(field)(obj);
						val = Field(val ?? default, name);
						if (Cell.current.valChanged)  field.SetValue(obj, val); 
					}
				}
				...
				else if (type == typeof(Color))
				{
					...
					{
						Color val = Getter<Color>.GetGetter(field)(obj);
						val = Field((Color)val, name);
						if (Cell.current.valChanged)  field.SetValue(obj, val); 
					}
				}
				...
				else if (type == typeof(double))
				{
					...
					{
						double val = Getter<float>.GetGetter(field)(obj);
						val = Field(val, name);
						if (Cell.current.valChanged)  field.SetValue(obj, val); 
					}
				}
				...
				{
					Enum val = (Enum)field.GetValue(obj);  //Getter<Enum>.GetGetter(att.field)(obj); Invalid IL code
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				{
					Vector2 val = Getter<Vector2>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				{
					Vector3 val = Getter<Vector3>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				{
					Vector4 val = Getter<Vector4>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				{
					Vector2D val = Getter<Vector2D>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				{
					Rect val = Getter<Rect>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				{
					Coord val = Getter<Coord>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}

				else if (type == typeof(CoordRect))
				{
					CoordRect val = Getter<CoordRect>.GetGetter(field)(obj);
					val = Field(val, name);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				else if (type == typeof(Texture2D))
				{
					...
					{		
						Texture2D val = Getter<Texture2D>.GetGetter(field)(obj);
						val = Field(val, name);
						if (Cell.current.valChanged)  field.SetValue(obj, val);
				...
				else if (type == typeof(Transform))
				{
					...
					{
						Transform val = Getter<Transform>.GetGetter(field)(obj);
						val = Field(val ?? default, name, allowSceneObject:allowSceneObject);
						if (Cell.current.valChanged)  field.SetValue(obj, val); 
					}
				}
				...
				else if (type == typeof(GameObject))
				{
					...
					{
						GameObject val = Getter<GameObject>.GetGetter(field)(obj);
						val = Field(val ?? default, name, allowSceneObject:allowSceneObject);
						if (Cell.current.valChanged)  field.SetValue(obj, val); 
					}
				}
				...
				{
					UnityEngine.Object val = Getter<UnityEngine.Object>.GetGetter(field)(obj);
					val = Field(val ?? default, name, type, allowSceneObject);
					if (Cell.current.valChanged && !UI.current.layout)  field.SetValue(obj, val); 
				}
				...
				else
				{
					...
					if (val != null) DualLabel(name, val.ToString());
					else DualLabel(name, "null");
				}
			}

			static class Getter<T>
			{
				static Dictionary<FieldInfo, Func<object,T>> cache = new Dictionary<FieldInfo, Func<object, T>>(); //TODO: make weak?
			...
			private static ValAttribute[] GetCachedVals (Type type)
			{
		...
		#region Other Elements
			...
			public static bool Button (string label, bool visible=true, GUIStyle style=null, MouseCursor cursor=0) 
			{
				...
				if (pressedButton == Cell.current  &&  Event.current.rawType == EventType.MouseUp)
				{
					...
					if (Event.current.button==0  &&  Cell.current.Contains(UI.current.mousePos))
					{
						UI.current.MarkChanged();
						return true;
					}
			...
			public static bool CheckButton (bool val, Texture2D iconOff, Texture2D iconOn, bool visible=true) 
			{
				val = CheckButton(val, visible:visible);
			...
			public static void Icon (Texture2D icon, Vector2 center, Color color = new Color(), float scale=1)
			/// Draws an icon in the given coord in native resolution (for zoom 1). Independent from cell
			{
				...
				if (color.a==0) UnityEngine.GUI.DrawTexture(rect, icon, ScaleMode.ScaleAndCrop);
				else UnityEngine.GUI.DrawTexture(rect, icon, ScaleMode.ScaleAndCrop, true, 0, color, 0,0);
			}

			public static void Icon  (Texture2D icon, Color color = new Color(), float scale=1)
			...
			public static void Texture (Texture2D texture, Material mat = null, ScaleMode scaleMode = ScaleMode.ScaleToFit)
			{
				if (UI.current.layout) return;
				if (UI.current.optimizeElements && !UI.current.IsInWindow()) return;
			...
			private static string LabelEditField (string label, GUIStyle style=null)
			{
			...
			private static Cell activeEditLabelCell; //to know what label we are currently editing
			...
			/// Draws grid in window-related rect. Moves with zoom. Useful for backgrounds
			{
				...
				Vector2 dispOffset = new Vector2(-UI.current.scrollZoom.scroll.x+displayRect.x, UI.current.scrollZoom.scroll.y-displayRect.height-displayRect.y); //-1 makes the line pass through 0-1 pixel
			...
			/// Draws grid in window-related rect
			{
				...
				gridMat.SetVector("_Rect", new Vector4(displayRect.x, displayRect.y, displayRect.size.x, displayRect.size.y) * dpiFactor);
			...
			/// Draws a mesh within a cell, presuming mesh fits in 0-1 bounds
			{
				...
				if (mat.HasProperty("_ClipRect"))
				{
					if (clip)
					{
						...
						Rect clipRect = CoordinatesExtensions.Intersect(windowRect, rect);
		...
		#region DragValue
			...
			public static float RaiseValue (float val, float deltaSteps, float exponentiality=1, float sensitivity=1000f) //actually un-sensitivity
			{
				float sign = val>=0? 1 : -1;
				float absVal = val*sign;

				float steps = Mathf.Log(absVal+exponentiality, 2f);
				steps += deltaSteps/sensitivity * sign;
			
				absVal = Mathf.Pow(2f, steps) - exponentiality;
