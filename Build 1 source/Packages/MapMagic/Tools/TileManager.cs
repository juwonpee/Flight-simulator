...
namespace Den.Tools
{
	...
	public class TileManager<T> : ISerializationCallbackReceiver where T: ITile//, IEquatable<T>
	{
		...
		#region Per-frame/Update
			...
			public void Update (Vector3 tileSize, MonoBehaviour holder=null, bool distsOnly=false)
			{
				...
				if (!distsOnly) Deploy(camCoords, holder:holder);
				Profiler.EndSample();
			...
			public void ReDeploy (Vector3 tileSize, MonoBehaviour holder=null)
			{
				RemoveNulls(); //excluding removed objects
				RefreshCamCoords(tileSize.x);
				Deploy(camCoords, holder:holder);
				ChangeDists(camCoords);
			}
