...
namespace Den.Tools
{
	public static class ScriptableAssetExtensions
	{
		...
		public static void ShowObjectSelector (Type objType, int id=12345, bool allowSceneObjects=false)
		{
			if (objectSelectorType == null) objectSelectorType = Type.GetType("UnityEditor.ObjectSelector,UnityEditor");
			object objectSelector = GetObjectSelector();

			MethodInfo showMethod = objectSelectorType.GetMethod("Show", BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
			showMethod.Invoke(objectSelector, new object[]{null, objType, null, allowSceneObjects});  //(obj, objType, property, allowSceneObjects);
