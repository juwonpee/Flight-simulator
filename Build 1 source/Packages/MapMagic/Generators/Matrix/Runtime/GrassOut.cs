...
namespace MapMagic.Nodes.MatrixGenerators
{
	...
	public class GrassOutput200 : Generator, IInlet<MatrixWorld>, IOutputGenerator, IOutput
	{
		...
		public static void Finalize (TileData data, StopToken stop)
		{
			...
			foreach ((GrassOutput200 output, MatrixWorld product, MatrixWorld biomeMask) 
				in data.finalize.ProductSets<GrassOutput200,MatrixWorld,MatrixWorld>(finalizeAction, data.subDatas))
			{
				if (stop!=null && stop.stop) return;

				detailArr[i] = CreateDetailLayer(product, biomeMask, output.density*product.PixelSize.x, i, data, stop);
			...
			//pushing to apply
			if (stop!=null && stop.stop) return;
			ApplyData applyData = new ApplyData() {detailLayers=detailArr, detailPrototypes=prototypes};
			Graph.OnBeforeOutputFinalize?.Invoke(typeof(GrassOutput200), data, applyData, stop);
		...
		private static int[,] CreateDetailLayer (Matrix matrix, Matrix biomeMask, float density, int randomNum, TileData data, StopToken stop)
		{
			...
			int arrSize = data.area.active.rect.size.x;
			int fullSize = data.area.full.rect.size.x;
			int margins = data.area.Margins;
			float pixelSize = data.area.PixelSize.x;
